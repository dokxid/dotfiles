  ...
  dependencies = {
    ...
    { "AstroNvim/astroui", opts = { icons = { Harpoon = "󱡀" } } },
    {
      ...
      opts = function(_, opts)
        ...
        maps.n["<C-,>"] = { function() require("harpoon"):list():prev() end, desc = "Goto previous mark" }
        maps.n["<C-.>"] = { function() require("harpoon"):list():next() end, desc = "Goto next mark" }
        for i = 0, 9, 1 do
          maps.n[prefix .. tostring(i)] =
            { function() require("harpoon"):list():select(i) end, desc = "open mark " .. tostring(i) }
        end
