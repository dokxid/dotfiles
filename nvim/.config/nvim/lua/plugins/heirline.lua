return {
  ...
  opts = function(_, opts)
    ...
    opts.statusline = { -- statusline
      hl = { fg = "fg", bg = "bg" },
      status.component.mode { mode_text = { padding = { left = 1, right = 1 } }, hl = function() 
      ...
      status.component.builder({
        {
          provider = function()
            ...
            return status.utils.stylize(time, {
              ...
              padding = { left = 2, right = 1 }, -- pad the right side so it's not cramped
            })
          end,
        },
        ...
        hl = function() 
          local hl = status.hl.get_attributes("mode")
          hl.bold = true
          return hl
        end, -- highlight based on mode attributes
        surround = { separator = "right", color = function() return require("astroui.status.hl").mode_bg() end, update = { "ModeChanged", pattern = "*:*" } }, -- background highlight based on mode
      }),
    }
